...
def embed_image(image_path):
    """Embed an image file as base64 in markdown"""
    try:
        ...
        return f"![{os.path.basename(image_path)}](data:{mime_type};base64,{encoded_image})"
    except Exception as e:
        return f"*Error embedding image {os.path.basename(image_path)}: {str(e)}*"


def create_markdown_note(models_dir, trial_num=None, include_visualizations=False):
    ...
    if trial_num is not None:
        ...
        if include_visualizations:
            # Look for common image formats in visualizations directory
            vis_extensions = [".png", ".jpg", ".jpeg", ".svg"]
            for ext in vis_extensions:
                vis_files = glob.glob(
                    os.path.join(trial_dir, "**", f"*{ext}"), recursive=True
                )
                files_to_process.extend(vis_files)
    else:
    ...
    for file_path in files_to_process:
        ...
        # Handle image files (if include_visualizations is True)
        image_extensions = [".png", ".jpg", ".jpeg", ".svg"]
        if include_visualizations and any(
            file_path.endswith(ext) for ext in image_extensions
        ):
            markdown_content += embed_image(file_path) + "\n\n"
            continue
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory to save output file (defaults to scripts/reports)",
    )
    parser.add_argument(
        "--include-visualizations",
        action="store_true",
        help="Include visualization images (as embedded base64) in the output markdown",
    )
    ...
    markdown_content = create_markdown_note(
        models_dir, args.trial, args.include_visualizations
    )
