...
def remove_comments(code):
    # Remove single line comments but preserve inline comments that might explain edge cases
    lines = code.split("\n")
    result = []
    for line in lines:
        ...
        if line.strip().startswith("#"):
            continue
        # Keep the code but remove trailing comments
        code_part = line.split("#", 1)[0]
        if code_part.strip():  # Only keep lines that have actual code
            result.append(line)
        else:
...
def is_minimal_file(code):
    # Check if file is minimal (like empty __init__.py)
...
def generate_directory_tree(
    ...
    if len(trial_dirs) > trial_limit:
        ...
        # Add an ellipsis item
        display_items = non_trial_items + limited_trials
        # We'll add an ellipsis entry later
    ...
    for i, (item, is_dir) in enumerate(dirs_and_files):
        ...
        if is_dir:
            # If this directory has too many items, summarize instead
            try:
...
def create_markdown(
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# Project Code Overview (Compact Version)\n\n")
        ...
        # Generate table of contents (only for non-minimal files if skip_minimal is True)
        non_minimal_files = []
        ...
        for file_path in python_files:
            ...
            non_minimal_files.append((file_path, code))

        # Write table of contents
        f.write("## Table of Contents\n\n")
        for file_path, _ in non_minimal_files:
        ...
        # Write each file with its processed content
        for file_path, code in non_minimal_files:
            rel_path = get_relative_path(file_path, base_dir)
            anchor = rel_path.replace("/", "-").replace(".", "-").replace(" ", "-")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compile Python files into a compact markdown document for LLM analysis, with optional removal of docstrings and comments."
    )
    parser.add_argument(
        "--directory",
        "-d",
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["src/app", "project_docs"],  # Added project_docs to ignore
    ...
    parser.add_argument(
        "--keep-comments", action="store_true", help="Keep comments in the output"
    )
    parser.add_argument(
        "--include-minimal",
        action="store_true",
        help="Include minimal files like empty __init__.py",
    )
    parser.add_argument(
        "--no-project-structure",
        action="store_true",
        help="Exclude predefined project structure from the output",
    )
    ...
    create_markdown(
        ...
        base_dir,
        args.output,
        remove_docs=not args.keep_docstrings,
        remove_cmts=not args.keep_comments,
        skip_minimal=not args.include_minimal,
        include_project_structure=not args.no_project_structure,
    )

    print(f"Compact markdown document created at {args.output}")
